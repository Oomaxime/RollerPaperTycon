...
export default function GameScreen() {
  ...
  const [bigIntScore, setBigIntScore] = useState<bigint>(0n);
  // On initialise avec une couleur par défaut
  ...
  // Récupération de la couleur stockée dans le localStorage avec une valeur par défaut
  useEffect(() => {
    if (typeof window !== "undefined") {
    ...
    <div className="flex flex-col items-center min-h-screen overflow-hidden">
      {isConnected ? (
        <div className="flex flex-col items-center gap-6 w-full p-5 min-h-screen">
          ...
          <Button onClick={handleMintClick}>Convertir en RPT</Button>
          <div className="flex w-full justify-end items-center absolute bottom-4 right-8 gap-2">
            <p className="text-gray-900 font-medium text-xl">{balance?.toString()} RPT</p>
        ...
        <div className="flex flex-col items-center gap-10 p-8 relative">
          ...
          <p className="text-gray-800 text-center font-bold text-2xl">
            Hola cowboy ! 🤠 Tu veux dérouler du papier toilette ?! Devenir l'heureux propriétaire des plus grand rouleaux de ce monde ?!
          </p>
          <ConnectButton label="Connecte toi !" accountStatus="avatar" chainStatus="none" />
          <ArrowAnimation />
        </div>
      )}
    </div>
